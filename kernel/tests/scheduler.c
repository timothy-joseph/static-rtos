...
static uint16_t           kticks = 0;
static struct mcu_context kscheduler_context;
                      ...
                      size_t stack_size, uint8_t priority)
{
	...
	/* scheduler calls all of the heapifying */
	__heapify();
 ...
 * algorithm:
 *  1. increment ticks
 *  2. unsuspend the tasks scheduled for unsuspending at
 *     this tick value
 *  3. heapify
 *  4. yield to the scheduler if needed
 */
void
kscheduler_tick_isr(void)
{
	...
	/* TODO: unsuspend tick tasks */

	__heapify();

	/* if higher priority task was readied, then yield */
